...
# communication
# root node receives results with a Reduce and SUM
# temporary array for sending array = local_array

# array for recieving
global_array = np.zeros((2, Ny, Nx)) # recvbuf
comm.Reduce(local_array, global_array, op = MPI.SUM, root = 0)

print("rank:", rank, "finished\n")
fig = plt.figure(figsize = (12,6))
plt.scatter(local_array[0,:], local_array[1,:], marker = '.', c = '#348ABD')
# Add text showing rank, and set plot limits
plt.title("local result")
...
# if a gather is used, the recieve array's size must be equal to the send array times number of ranks
# if rank == 0:
#    recvbuf = np.zeros((size, 2, Ny, Nx))
#comm.Gather(sendbuf, recvbuf, root=0)
# optional "check" of arrays
#if rank == 0:
#    for i in range(size):
#        assert np.allclose(recvbuf[i,:], i)

#comm.gather(X1[:,:,:], None, root=0)
		
# optional for debugging/preview: wait for all processes to finish
comm.Barrier()
 
# prints global results from root node

if comm.rank == 0:
    print("all processes finished")
    fig = plt.figure(figsize = (12,6))
    plt.scatter(global_array[0,:], global_array[1,:], marker = '.', c = '#348ABD')
    # Add text showing rank, and set plot limits
    plt.title("global result")
    plt.text(1.65, 0.9, 'rank = %s' % rank, size = 22)
    plt.xlim(0,2)
    plt.ylim(0,1)
    plt.show()